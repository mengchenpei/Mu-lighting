...
class GetFile:
    ...
    def onTimeout(self,interest):
        ...
        if(self.m_retryCount <= MAX_RETRY):
            ...
            self.oStream.close()
	    '''print "after close stream"
	    self.oStream = open(dataName+str("-os.txt"),'wb')
	    print"before start fetching ostxt",dataName+str("-os.txt"),
	    self.startFetchingData(Name(self._txtName).append(self.zero))
	    self.oStream.close()
	    self.oStream = open(dataName+str("-freq.txt"),'wb')
	    self.startFetchingData(Name(self._freqName).append(self.zero))
	    self.oStream.close()'''
	    self._playFunction()
